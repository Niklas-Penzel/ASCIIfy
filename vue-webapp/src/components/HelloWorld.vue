<template>
  <div class="row">
    <div class="column left" style="background-color:#9c0d38;">
      ...
      <Multiselect style="color:#FFFFFF"
        :canDeselect=false
      ...
      <Toggle v-model="keepRes" 
      @change="handleResize"
      />
      </div>
      <br>
      <br>
      <Slider v-model="resize" 
      :disabled="keepRes"
      :min=1
...
export default {
  ...
  methods:{
      ...
      loadFonts(){
        var self = this;
        axios.post(`${API_URL}/api/Fonts`, null)
      ...
      sendRequest () {
        if(this.uploadedImage != ''){
          ...
          payload.append("image", this.uploadedImage)
          payload.append("fontsize", this.size)
          payload.append("font", this.selectedFont)
          payload.append("resize", this.resizeFactor)

          var self = this;
          axios.post(`${API_URL}/api/Asciify`, payload)
          .then(function (response) {
            console.log(response)
            if(self.uploadedImage != ''){
              console.log(response)
              self.asciifiedImage = 'data:image/jpeg;base64,' + response.data
            }
          })
          .catch((error) => {
            console.log(error);
