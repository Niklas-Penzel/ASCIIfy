...
def ASCIIfy(img, resize, font_path, font_size, stroke_width, verbose=1):
    ...
    ascii_img = np.zeros(
        (img_resized.shape[0] * symbol_height, img_resized.shape[1] * symbol_width, 3),
        dtype=np.uint8,
    )
    ...
    if verbose > 0:
        it = tqdm(
            np.ndindex(img_resized.shape[:2]),
            total=img_resized.shape[0] * img_resized.shape[1],
        )
    else:
    ...
    for idx in it:
        ...
        # render the text
        editable_ascii_img.text(
            anchor, char, fill=color, font=font, stroke_width=stroke_width
        )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="A script that can be used to asciify one image or multiple images."
    )
    parser.add_argument(
        "in_path",
        type=str,
        help="Path to the raw data. Can be a single image or a directory containing multiple images.",
    )
    parser.add_argument(
        "out_path",
        type=str,
        help="Path to where you want to save the converted image(s).",
    )
    parser.add_argument(
        "--font",
        default=os.path.join("fonts", "MajorMonoDisplay-Regular.ttf"),
        type=str,
        help="Path to font file, USE MONO!",
    )
    parser.add_argument(
        "--fontsize", default=10, type=int, help="Which font size should be used?"
    )
    parser.add_argument(
        "--boldness",
        default=0,
        type=int,
        help="How bold should the letters be? Increase, if the image is too dark.",
    )
    parser.add_argument(
        "--resize",
        default=-1,
        type=int,
        help="By how much should the image be resized before asciifying it. If -1 it will choose a resize corresponding to the fontsize so that the resulting image has approximately the same dimensions as the input.",
    )
    parser.add_argument(
        "--n_workers",
        default=-1,
        type=int,
        help="How many processes should be started? If -1 it uses the number of cores.",
    )
    ...
    if os.path.isfile(args.in_path):
        ...
        if args.n_workers == 1:
            for f in tqdm(os.listdir(args.in_path)):
                try:
                    ASCIIfy_(
                        os.path.join(args.in_path, f),
                        os.path.join(args.out_path, f),
                        args.resize,
                        args.font,
                        args.fontsize,
                        args.boldness,
            ...
            asciifier = ASCIIfier(
                args.resize, args.font, args.fontsize, args.boldness, verbose=0
            )
            if args.n_workers < 0:
                args.n_workers = mp.cpu_count()

            paths = [
                (os.path.join(args.in_path, f), os.path.join(args.out_path, f))
